...
function drawCanvas() {
  // 讓 canvas 隨容器縮放
  editorCanvas.width = editorCanvas.offsetWidth;
  editorCanvas.height = editorCanvas.offsetHeight;
  ctx.fillStyle = '#fff';
  ctx.fillRect(0, 0, editorCanvas.width, editorCanvas.height);

  // 畫頁面底紋
  const page = currentBook.content[currentPage];
  if (currentBook.style === 'lined') {
    ctx.strokeStyle = "#e3e3e3";
    for (let y = 30; y < editorCanvas.height; y += 25) {
      ctx.beginPath();
      ctx.moveTo(0, y);
      ctx.lineTo(editorCanvas.width, y);
      ctx.stroke();
    }
  } else if (currentBook.style === 'grid') {
    ctx.strokeStyle = "#e3e3e3";
    for (let y = 30; y < editorCanvas.height; y += 25) {
      ctx.beginPath();
      ctx.moveTo(0, y);
    ...
    for (let x = 30; x < editorCanvas.width; x += 25) {
      ctx.beginPath();
      ctx.moveTo(x, 0);
...
editorCanvas.addEventListener('touchstart', function (e) {
  ...
  // 讓這次事件轉為 mousedown
  const fakeMouseEvent = new MouseEvent('mousedown', { clientX: t.clientX, clientY: t.clientY });
  editorCanvas.dispatchEvent(fakeMouseEvent);
  e.preventDefault();
}, { passive: false });
...
editorCanvas.addEventListener('touchmove', function (e) {
  ...
  const rect = editorCanvas.getBoundingClientRect();
  const t = e.touches[0];
  const fakeMouseEvent = new MouseEvent('mousemove', { clientX: t.clientX, clientY: t.clientY });
  editorCanvas.dispatchEvent(fakeMouseEvent);
  e.preventDefault();
}, { passive: false });
